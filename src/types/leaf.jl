...
mutable struct LeafBiophysics{FT<:AbstractFloat}
    ...
    LeafBiophysics{FT}(wls::WaveLengthSet{FT}; opti::String=OPTI_2021) where {FT<:AbstractFloat} = (
        ...
        return new{FT}(1, 0.012, 0, 0, _Kant, _Kbrown, _Kcab, _KcaV, _KcaZ, _Kcbc, _Kh2o, _Klma, _Kpro, _Kps, 1.4, 10, _nr, 0, 0, 40, 10, 0, 0.01, zeros(FT,NΛ_SIF,NΛ_SIFE), zeros(FT,NΛ_SIF,NΛ_SIFE),
                       zeros(FT,NΛ), zeros(FT,NΛ), zeros(FT,NΛ), 0.01, zeros(FT,NΛ), 0.01, zeros(FT,NΛ), zeros(FT,NΛ))
    )
end
...
# General
#     2021-Nov-11: add photosynthesis system structure
#
#######################################################################################################################################################################################################
mutable struct LeafPhotosynthesisSystem{FT<:AbstractFloat}
    # parameters that do not change with time
...
mutable struct LeafPhotosynthesis{FT<:AbstractFloat}
    ...
    # SYSTEM::LeafPhotosynthesisSystem{FT}
    ...
    # constructors
    LeafPhotosynthesis{FT}() = new{FT}(0.5, 0.05, 4, 0.85, 4/4.9, 20, 30, 40, 4, 0, 0)
end
...
mutable struct Leaf{FT<:AbstractFloat}
    ...
    # constructors
    Leaf{FT}(wls::WaveLengthSet{FT}; opti::String=OPTI_2021) where {FT<:AbstractFloat} = (
        return new{FT}(LeafBiophysics{FT}(wls; opti=opti), LeafPhotosynthesis{FT}(), 298.15, saturation_vapor_pressure(298.15), 0.0)
    );

    Leaf{FT}(swl::Vector{FT}=FT.(WAVELENGTHS); opti::String=OPTI_2021) where {FT<:AbstractFloat} = (
        return new{FT}(LeafBiophysics{FT}(swl; opti=opti), LeafPhotosynthesis{FT}(), 298.15, saturation_vapor_pressure(298.15), 0.0)
    );
end
